...
/**
 * Sample transaction
 * @param {org.trade.net.SampleTransaction} sampleTransaction
 * @transaction
 */
async function sampleTransaction(tx) {
    ...
    const assetRegistry = await getAssetRegistry('org.trade.net.SampleAsset');
    // Update the asset in the asset registry.
    await assetRegistry.update(tx.asset);

    // Emit an event for the modified asset.
    let event = getFactory().newEvent('org.trade.net', 'SampleEvent');
...
/**
 ...
 function onMergeAssets(tx) {
    ...
    return getAssetRegistry('org.trade.net')
      .then(function(ar) {
        assetRegistry = ar;
